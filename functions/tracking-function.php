  ...
  foreach($all_plugins as $key => $value)
	{
	 ...
	 if(is_plugin_active($mykey))
	 {
		...
		'body' => array( 'user_name' => $name, 'user_email' =>$email,'site_url' => $site_url,
						...
						'name_outdated_theme' => 'outdated','name_outdated_plugins' => 'outdated', 'user_contact' => $email,'action'=>'activate' ),
		'cookies' => array()
		)
	);
...
function mydeactivationfunction($newname, $newtheme) { $current_user = wp_get_current_user();
	 $name=$current_user->user_login;
    $email=$current_user->user_email;
    $site_url=get_site_url();
    $active_theme=wp_get_theme();
  ...
  foreach ( $themes as $theme ) {
	  ...
	  if($theme != $active_theme)
	  {
		...
		'body' => array( 'user_name' => $name, 'user_email' =>$email,'site_url' => $site_url,
						...
						'name_of_active_plugins' => $active_plugins,'name_of_dective_plugins' => $inactive_plugins,
						'name_of_theme_inactive' => $inactive_themes,'version_of_wp' =>  $wp_version, 
						'name_outdated_theme' => '','name_outdated_plugins' => '', 'user_contact' => $email,'action'=>'deactivate' ),
		'cookies' => array()
		)
	);
